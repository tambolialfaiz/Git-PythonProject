...
root.resizable(0,0)
root.title("AT:Message Encode and Decode")

Label(root, text ='ENCODE DECODE', font = 'arial 20 bold').pack()

Label(root, text ='AT', font = 'arial 20 bold').pack(side =BOTTOM)
...
def Encode(key,messege):
    enc=[]
    for i in range(len(messege)):
        key_c=key[i%len(key)]
        enc.append(chr((ord(messege[i]) + ord(key_c)) % 256))
